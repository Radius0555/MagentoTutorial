...
class Dialcom_Przelewy_Adminhtml_PrzelewyController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function paymentemailAction()
    {
        ...
        $base_url = Mage::app()->getStore($this->storeID)->getBaseUrl();
        try {
            ...
            $right_key = md5($order->getStoreId() . '|' . $order->getIncrementId());

            $order->payment_link = $base_url.'przelewy/przelewy/summary?order_id='.$order->getIncrementId().'&key='.$right_key;

            $vars = array(
                'order' => $order,
                'yourOrder' => $this->translateInStore($storeId, 'Your order'),
                'click' => $this->translateInStore($storeId, 'To pay with Przelewy24 please click this'),
                'title' => $this->translateInStore($storeId, 'Payment for order')
            );
